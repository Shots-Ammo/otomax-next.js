...
/* ── Time slots: 1 PM – 4 PM, every 30 min ──────────────── */
const timeSlots = [
  "1:00 PM", "1:30 PM",
  "2:00 PM", "2:30 PM",
  "3:00 PM", "3:30 PM",
...
function getNextTenDays(locale: string) {
  ...
  while (count < 10) {
    ...
    days.push({
      ...
      disabled: count < 2,  // first 2 days grayed out
    });
    count++;
  }
  return days;
...
/* ════════════════════════════════════════════════════════════
   COMPONENT
════════════════════════════════════════════════════════════ */
export function WebsiteOnboardingModal({ isOpen, onClose }: WebsiteModalProps) {
  const totalSteps = 5;
  ...
  const submit = async () => {
    ...
    // ════════════════════════════════════════════════════════════
    // TARGET: Website Form
    // URL: https://n8n.srv1587679.hstgr.cloud/webhook/website-form
    // ════════════════════════════════════════════════════════════
    try {
      await fetch("https://n8n.srv1587679.hstgr.cloud/webhook/website-form", {
        method: "POST",
        headers: {
  ...
  /* step-specific "next" guard */
  const canNext = () => {
    if (step === 1) return formData.businessName.trim().length > 1;
    if (step === 2) return !!(formData.instagram || formData.twitter || formData.tiktok);
  ...
  const titles = t.raw("website.titles");
  const subs = t.raw("website.subs");
          ...
          <AnimatePresence mode="wait">
            {isSubmitted ? (
              ...
              >
                ...
                <h2 className="text-3xl font-black text-neutral-900">{t("website.successTitle")}</h2>
                <p className="text-neutral-500 leading-relaxed px-4 text-start">
                  {t.rich("website.successMsg", {
                    date: formData.date,
                    time: formData.time,
                    email: formData.email,
                    span: (chunks) => <span className="text-[#7B2FFF] font-bold">{chunks}</span>,
              ...
              >
                <div className="space-y-1 mb-2 text-start">
                  ...
                  <p className="text-[#7B2FFF] font-bold text-sm tracking-widest uppercase pt-2">
                ...
                {step === 1 && (
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-neutral-50 border border-neutral-200 rounded-2xl py-5 pl-12 pr-4 text-neutral-900 font-semibold outline-none focus:ring-2 focus:ring-[#7B2FFF]/25 focus:border-[#7B2FFF] transition-all"
                    />
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <input autoFocus type="text" name="instagram" value={formData.instagram} onChange={handle}
                        ...
                        className="w-full bg-neutral-50 border border-neutral-200 rounded-xl py-4 pl-12 pr-4 text-neutral-900 font-semibold outline-none focus:ring-2 focus:ring-[#7B2FFF]/25 focus:border-[#7B2FFF] transition-all"
                      />
                    </div>
                    <div className="relative">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3">
                      <select name="countryCode" value={formData.countryCode} onChange={handle}
                        className="w-[120px] shrink-0 bg-neutral-50 border border-neutral-200 rounded-xl py-4 px-3 text-neutral-900 font-semibold appearance-none outline-none focus:ring-2 focus:ring-[#7B2FFF]/25 focus:border-[#7B2FFF]"
                      >
                        {countries.map(c => (
